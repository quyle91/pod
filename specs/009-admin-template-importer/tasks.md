...
- [x] **TASK-919**: **Product-Centric Meta Box Architecture**: Loại bỏ khái niệm Template độc lập và trang Import riêng; tích hợp trực tiếp toàn bộ luồng bóc tách PSD (Chunked Uploader 5MB sang Node.js, Polling Worker trạng thái, Visual Cây Layer 100% Layer-driven, ánh xạ Asset Category dropdown, và trình chỉnh sửa JSON) ngay trong Meta Box *"🎨 Thiết Kế POD: Bóc Tách Layer Từ Photoshop (.PSD)"* trên màn hình chỉnh sửa sản phẩm WooCommerce (`post.php?post={id}&action=edit`), lưu trực tiếp cấu hình vào meta `_pod_template_config`.
- [x] **TASK-920**: Kiểm thử tích hợp E2E: Upload file PSD trực tiếp trong Product Meta Box $\rightarrow$ Bóc tách layer 300 DPI qua Node.js Worker $\rightarrow$ Hiển thị cây layer & biến thể swatch da/tóc $\rightarrow$ Khách đặt hàng trên Storefront không lỗi canvas $\rightarrow$ File in 300 DPI xuất xưởng chuẩn xác từ `sharpRenderer.js`.
